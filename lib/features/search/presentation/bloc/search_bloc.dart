...
class SearchBloc extends Bloc<SearchEvent, SearchState> {
  ...
  /// Bumped every time [_search] starts a fresh result set. An in-flight
  /// `LoadMoreResults` whose captured generation no longer matches drops
  /// its page instead of appending it to results it no longer belongs to.
  int _generation = 0;
  ...
  SearchBloc(this._repository) : super(const SearchState()) {
    ...
    on<SourcesRequested>(
      _onSourcesRequested,
      transformer: debounceRestartable(Duration.zero),
    );
  ...
  Future<void> _onStarted(
    ...
    final (topics, covers) = await (
      _repository.getTopics().orFallback(state.topics),
      _guarded(() => _repository.getSectionCovers(), state.sectionCovers),
    ).wait;
    emit(state.copyWith(topics: topics, sectionCovers: covers));
  }
  ...
  Future<void> _onQueryChanged(
    ...
    final suggestions = await _repository
        .getSuggestions(query)
        .orFallback(const <String>[]);
  ...
  Future<void> _onLoadMore(
    ...
    try {
      ...
      // was meant to extend; appending it now would corrupt them.
      if (generation != _generation) return;
      emit(
        state.copyWith(
  ...
  /// Like `orFallback`, but also absorbs a synchronous throw from the call.
  static Future<T> _guarded<T>(Future<T> Function() call, T fallback) async {
    try {
      return await call();
    } catch (_) {
      return fallback;
