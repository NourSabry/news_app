...
class FeedBloc extends Bloc<FeedEvent, FeedState> {
  ...
  /// `RefreshFeed`, which are continuations of the current load.
  ...
  Future<void> _onLoadMore(LoadMoreFeed event, Emitter<FeedState> emit) async {
    ...
    try {
      ...
      // meant to extend — merging it now would corrupt the new feed.
      if (generation != _generation) return;
      emit(
        state.copyWith(
  ...
  Future<void> _onRefresh(RefreshFeed event, Emitter<FeedState> emit) async {
    ...
    try {
      ...
      // Same guard as load-more: a topic/scope change mid-flight
      // means this delta no longer applies to the feed now shown.
      if (generation != _generation) return;
      emit(
        state.copyWith(
          articles: _applyDelta(state.articles, delta),
          pendingArticles: _merge(
            _unseen(delta.newArticles),
  ...
  /// — a refresh that removed something is never reported as a no-op.
