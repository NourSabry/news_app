...
Article article(String id, {List<ContentBlock>? body, List<String>? related}) => Article(
      id: id,
      title: 'Title $id',
      summary: 'Summary',
      source: 'Source',
...
void main() {
  ...
  setUp(() {
    ...
    when(() => repository.fetchRelated(['b'])).thenAnswer((_) async => [article('b')]);
  });
  ...
  blocTest<DetailsBloc, DetailsState>(
    ...
    build: () {
      ...
      when(() => repository.fetchArticle('a')).thenAnswer((_) async => ArticleFound(full));
  ...
  blocTest<DetailsBloc, DetailsState>(
    ...
    build: () {
      ...
      when(() => repository.fetchArticle('a'))
          .thenAnswer((_) async => const ArticleUnavailable('removed_by_publisher'));
      return DetailsBloc(repository, article('a', related: const ['b']));
    },
    act: (bloc) => bloc.add(const LoadArticle()),
    expect: () => [
      DetailsState(article: article('a', related: const ['b']), isLoading: true),
      DetailsState(
  ...
  blocTest<DetailsBloc, DetailsState>(
    ...
    expect: () => [
      DetailsState(article: preview, isLoading: true),
      DetailsState(article: preview, errorMessage: DetailsBloc.loadErrorMessage),
    ],
