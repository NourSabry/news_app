...
Article article({bool isLiked = false, int likes = 10, int version = 1}) =>
    Article(
      id: 'a',
      title: 'Title',
      summary: 'Summary',
...
ReactionsState overridden({
  ...
  return ReactionsState(
    overrides: {
      'a': ArticleOverrides(isLiked: isLiked, likes: likes, version: version),
    },
...
void main() {
  ...
  setUpAll(() {
    registerFallbackValue(
      const ArticleOverrides(isLiked: false, likes: 0, version: 1),
    );
  });
  ...
  setUp(() {
    ...
    when(
      () => repository.persistOverride(any(), any()),
    ).thenAnswer((_) async {});
  });
  ...
  blocTest<ReactionsBloc, ReactionsState>(
    ...
    build: () {
      when(
        () => repository.toggleLike('a', expectedVersion: 1),
  ...
  blocTest<ReactionsBloc, ReactionsState>(
    ...
    build: () {
      when(
        () => repository.toggleLike('a', expectedVersion: 3),
  ...
  blocTest<ReactionsBloc, ReactionsState>(
    ...
    expect: () => [
      ...
      overridden(
        ...
        likes: 186,
        version: 3,
        notice: ReactionsBloc.conflictMessage,
      ),
    ],
  );
  ...
  blocTest<ReactionsBloc, ReactionsState>(
    ...
    act: (bloc) =>
        bloc.add(ToggleLike(article(isLiked: true, likes: 186, version: 3))),
    expect: () => [
  ...
  blocTest<ReactionsBloc, ReactionsState>(
    ...
    verify: (_) =>
        verify(() => repository.toggleLike('a', expectedVersion: 1)).called(1),
  );
}
