...
void main() {
  ...
  test(
    'retains bookmarks and like counts across construction over the same store',
    () async {
      final store = MemoryStore();
      final first = MockApiClient(store: store)..latencyMs = 0;

      await first.setBookmark(
        articleId: 'a_flutter_roadmap',
        bookmarked: false,
      );
      await first.setBookmark(
        articleId: 'a_battery_breakthrough',
        bookmarked: true,
      );
  ...
  test('search also matches on source name and author name', () async {
    ...
    expect(
      bySource.data.map((a) => a.id),
      containsAll(['a_architecture_patterns', 'a_ai_policy']),
    );
  ...
  test(
    'a real feed article is reported deleted on the second refresh, not the first',
    () async {
      final client = MockApiClient(store: MemoryStore())..latencyMs = 0;
  ...
  test(
    'syncOutbox produces a conflict for a like when simulateConflict is on',
    () async {
      ...
      final response = await client.syncOutbox(
        baseVersion: 0,
        mutations: [mutation],
      );
      ...
      expect(
        article.isLiked,
        isFalse,
        reason: 'a rejected mutation must not be applied server-side',
      );
    },
  );

  test(
    'outbox_version persists across construction over the same store',
    () async {
      final store = MemoryStore();
      final first = MockApiClient(store: store)..latencyMs = 0;
      final mutation = OutboxEntry(
      ...
      final firstResponse = await first.syncOutbox(
        baseVersion: 0,
        mutations: [mutation],
      );
      ...
      final second = MockApiClient(store: store)..latencyMs = 0;
      final secondResponse = await second.syncOutbox(
        baseVersion: 1,
        mutations: [],
      );
  ...
  test(
    'resetServerState clears persisted bookmarks and article overrides',
    () async {
      final store = MemoryStore();
      final first = MockApiClient(store: store)..latencyMs = 0;
      await first.setBookmark(
